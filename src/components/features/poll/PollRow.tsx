...
import { useDeletePollMutation } from "@/services/poll.services";
...
const getStatusBadge = (status: string) => {
  const statusStyles = {
    ...
    "in progress":
      "flex justify-center items-center border text-green-600 border-green-600 w-20",
...
export const PollRow: React.FC<PollRowProps> = ({ polls = [] }) => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deletePoll(Number(id)).unwrap();
      toast({
        title: "Succès",
    ...
    <>
      {polls.map((poll) => (
        <TableRow key={poll.id}>
          <TableCell className="text-left font-medium w-[30%]">
            {poll.title}
          </TableCell>
          ...
          <TableCell className="text-left w-[15%]">
            {formatDate(poll.dateDebut)}
          </TableCell>
          <TableCell className="text-left w-[15%]">
            {formatDate(poll.dateFin)}
          </TableCell>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        définitivement le sondage "{poll.title}" et toutes les
                        données associées.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
